...
class PasswordValidationPlusState extends State<PasswordValidationPlus> {
  RegExp numReg = RegExp(r".*[0-9].*"); //use to find password text contains at least one digit
  RegExp simpleReg = RegExp(r".*[a-z].*"); //use to find password text contains at least one simple letter
  RegExp capitalReg = RegExp(r".*[A-Z].*"); //use to find password text contains at least one CAPITAL letter
  RegExp symbolsReg = RegExp(r'.*[!@#\$&*~].*'); //use to find password text contains at least one symbol (!, @, #, \, $, &, *, ~)
  double _strength = 0;
  ...
  checkLength() {
    if (widget.maxLength != null) {
      return widget.textController.text.length >= widget.minLength &&
          widget.textController.text.length <= widget.maxLength!; //check maximum character length if maxLength field is not null
    } else {
      return widget.textController.text.length >= widget.minLength;
    }
  }
